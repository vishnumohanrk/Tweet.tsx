...
export const TweetIntent = ({ id }: { id: string }) => {
  const intentArr = [
    ...
    },
    {
      ...
      color: 'text-twitter hover:bg-twitter/10 focus-visible:bg-twitter/10',
    },
  ] as const;

  return (
    <div className="flex p-1 sm:px-4 justify-around text-secondaryText">
      {intentArr.map(i => (
        <TwitterLink
          key={i.name}
          path={i.link}
          aria-label={i.name}
          title={i.name === 'View on Twitter' ? i.name : undefined}
          className={`p-3 rounded-full transition-colors ${i.color}`}
        >
